...
export default function AdminApproved() {
  const { data, mutate } = useSWR('/api/modules?status=Approved', () => getModules('Approved'));
  const modules = data?.modules || [];

  if (!data) return <div className="p-6">Loading...</div>;
  if (modules.length === 0) return <div className="p-6">No approved modules</div>;

  return (
    <div className="p-6">
      <h2 className="text-xl font-semibold mb-4">Approved Modules</h2>
